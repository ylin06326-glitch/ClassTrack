...
const State = {
  ...
  privacyMode: false,  // 全局隐私模式：仅显示学号
};

/** 生成带作业种类ID的缓存key */
function cacheKey(date) {
  return `${date}_${State.currentHomeworkTypeId}`;
}
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // 恢复全局隐私模式
  try {
    const saved = localStorage.getItem("classtrack_privacy");
    if (saved === "1") {
      State.privacyMode = true;
      const toggle = document.getElementById("globalPrivacyToggle");
      if (toggle) toggle.checked = true;
    }
  } catch (e) {}

  // 全局隐私开关事件
  const globalPrivacyToggle = document.getElementById("globalPrivacyToggle");
  if (globalPrivacyToggle) {
    globalPrivacyToggle.addEventListener("change", () => {
      State.privacyMode = globalPrivacyToggle.checked;
      try { localStorage.setItem("classtrack_privacy", State.privacyMode ? "1" : "0"); } catch (e) {}
      refreshCurrentView();
    });
  }
...
function switchTab(tabName) {
  const map = { grouping: "tabGrouping", homework: "tabHomework", export: "tabExport", analytics: "tabAnalytics" };
  const prevActive = document.querySelector(".tab-content.active");
  const nextSec = document.getElementById(map[tabName]);

  if (prevActive && prevActive !== nextSec) {
    // v2: 旧 tab 退出动画，结束后再切换
    prevActive.classList.add("fading-out");
    prevActive.addEventListener("animationend", function handler() {
      prevActive.classList.remove("active", "fading-out");
...
async function loadHomeworkTypes() {
  const res = await API.get(`/api/homework-types?class_id=${State.currentClassId}`);
  if (res.code === 0) {
...
function setupHomeworkTypeSelectors() {
  ...
  // 管理按钮
  const btnManage = document.getElementById('btnManageHWTypes');
  if (btnManage) btnManage.addEventListener('click', openHWTypeModal);

  // Modal 事件
  document.getElementById('btnHWTypeModalClose').addEventListener('click', () => closeModal('hwTypeModal'));
  document.getElementById('btnAddHWType').addEventListener('click', addHWType);
  document.getElementById('newHWTypeName').addEventListener('keydown', e => { if (e.key === 'Enter') addHWType(); });
}

function refreshHWTypeList() {
  const list = document.getElementById('hwTypeList');
  if (!list) return;
  list.innerHTML = State.homeworkTypes.map(t => `
    <div class="class-item">
      <span class="class-item-name">${escapeHtml(t.name)} ${t.is_preset ? '<span class="class-btn-active">预设</span>' : ''}</span>
      <div class="class-item-actions">
...
function openHWTypeModal() {
  refreshHWTypeList();
  document.getElementById('newHWTypeName').value = '';
  document.getElementById('hwTypeModal').style.display = '';
}

async function addHWType() {
  const input = document.getElementById('newHWTypeName');
  const name = input.value.trim();
  if (!name) { showToast('请输入种类名称', 'error'); return; }
  const res = await API.post('/api/homework-types', { name, class_id: State.currentClassId });
  if (res.code === 0) {
...
async function deleteHWType(tid) {
  if (!confirm('删除作业种类将同时删除该种类下的所有作业记录，确定继续？')) return;
  const res = await API.del(`/api/homework-types/${tid}`);
  if (res.code === 0) {
    showToast(res.msg, 'success');
    await loadHomeworkTypes();
    if (State.activeTab === 'homework') renderHomeworkView(true);
    if (State.activeTab === 'analytics') loadAnalytics();
  } else showToast(res.msg, 'error');
}
...
async function loadStats(statsData) {
  const res = statsData ? { code: 0, data: statsData } : await API.get(`/api/stats?class_id=${State.currentClassId}`);
  if (res.code === 0) updateStatsDisplay(res.data);
}

async function loadHomeworkForDate(date) {
  const res = await API.get(`/api/homework?date=${date}&class_id=${State.currentClassId}&homework_type_id=${State.currentHomeworkTypeId}`);
  if (res.code === 0) State.homeworkCache[cacheKey(date)] = res.data;
}
...
function updateStatsDisplay(data) {
  ...
  document.getElementById("statRecords").textContent = data.total_homework_records || 0;
  const lockDot = document.querySelector(".lock-dot");
  const lockText = document.getElementById("lockStatusText");
  if (data.last_lock_time && data.last_lock_time !== "尚未锁定") {
    State.isLocked = true;
...
function setupGroupControls() {
  ...
  document.getElementById("btnResetGroups").addEventListener("click", async () => {
    ...
    await API.post(`/api/groups/reset?class_id=${State.currentClassId}`);
    showToast("已重置", "info"); await applyGroupCount(); await loadAllData();
...
async function saveCurrentGrouping() {
  ...
  for (const col of cols) {
    ...
    const studentIds = [...cards].map(c => parseInt(c.dataset.studentId));
    promises.push(API.put("/api/students/batch-move", { student_ids: studentIds, group_id: gid }));
  }
  // 未分组池中的学生统一归零
  const poolCards = document.querySelectorAll("#unassignedPool .student-card");
  if (poolCards.length > 0) {
    const poolIds = [...poolCards].map(c => parseInt(c.dataset.studentId));
    promises.push(API.put("/api/students/batch-move", { student_ids: poolIds, group_id: 0 }));
  }
...
function setupHomeworkControls() {
  ...
  document.querySelectorAll(".grade-batch-btn").forEach(btn => {
    btn.addEventListener("click", async () => {
      ...
      if (!date) return;
      if (!confirm(`确定将 ${date} 所有学生作业等级批量设为「${grade === "X" ? "未交" : grade}」吗？`)) return;
      ...
      if (res.code === 0) { showToast("批量设置成功", "success"); State.homeworkCache[cacheKey(date)] = null; debouncedLoadStats(); renderHomeworkView(); }
      else showToast(res.msg, "error");
    });
  });
}
...
async function renderHomeworkView(forceRebuild = false) {
  ...
  if (!State.homeworkCache[cacheKey(date)]) await loadHomeworkForDate(date);
  const records = State.homeworkCache[cacheKey(date)] || {};
  ...
  for (const g of State.groups) {
    ...
    for (const s of students) {
      ...
      const codeHtml = formatStudentCodeHtml(s.student_code);
      html += `<div class="hw-student-row" data-student-id="${s.id}"><span class="hw-student-name${privacyClass()}">${codeHtml}${escapeHtml(displayName)}</span><div class="grade-quick-select">`;
      for (const gv of ["A","B","C","X"]) {
        const gLabel = gv === "X" ? "未交" : gv;
...
async function _gradeClickHandler(e) {
  ...
  // 3. 后台发送API（失败回滚）
  try {
    const res = await API.post("/api/homework", { student_id: sid, date, grade, homework_type_id: State.currentHomeworkTypeId });
    if (res.code !== 0) {
...
async function setGradeDirect(studentId, grade, date) {
  ...
  // 后台API
  try {
    const res = await API.post("/api/homework", { student_id: studentId, date, grade, homework_type_id: State.currentHomeworkTypeId });
    if (res.code !== 0) {
...
async function openReminderModal() {
  const date = document.getElementById("homeworkDate").value;
  const res = await API.get(`/api/homework/missing?date=${date}&class_id=${State.currentClassId}&homework_type_id=${State.currentHomeworkTypeId}`);
...
function renderReminderList() {
  ...
  for (const s of _reminderData) {
    ...
    if (State.privacyMode && s.student_code) {
      byGroup[gname].push(s.student_code);
    } else {
      const label = s.student_code
        ? `${s.student_name}(${s.student_code})`
        : s.student_name;
      byGroup[gname].push(label);
    }
  }
  let listHtml = "";
  const privacyClass = State.privacyMode ? ' privacy' : '';
  for (const [gname, entries] of Object.entries(byGroup)) {
    listHtml += `<div class="reminder-group${privacyClass}"><strong>📌 ${escapeHtml(gname)}：</strong>${entries.map(n => escapeHtml(n)).join("、")}</div>`;
  }
...
function getReminderText() {
  ...
  for (const s of _reminderData) {
    ...
    if (State.privacyMode && s.student_code) {
      ...
      byGroup[gname].push(s.student_code ? `${s.student_name}(${s.student_code})` : s.student_name);
    }
  }
  let text = `【催交通知】${date}\n`;
  for (const [gname, entries] of Object.entries(byGroup)) {
...
function printReminder() {
  ...
  const privacyTitle = State.privacyMode ? '（仅显示学号）' : '';
...
function exportReminder() {
  window.open(`/api/export/class?homework_type_id=${State.currentHomeworkTypeId}&start=${document.getElementById("homeworkDate").value}&end=${document.getElementById("homeworkDate").value}&class_id=${State.currentClassId}`, "_blank");
}
...
async function prepareExportTab() {
  ...
  for (const s of State.students) {
    const displayName = formatStudentDisplay(s.name, s.student_code);
    const codeLabel = (!State.privacyMode && s.student_code) ? `[${s.student_code}] ` : '';
...
async function queryRecords() {
  ...
  const res = await API.get(`/api/homework/range?homework_type_id=${State.currentHomeworkTypeId}&start=${start}&end=${end}&class_id=${State.currentClassId}`);
...
function renderPreviewTable(records) {
  ...
  tbody.innerHTML = records.slice(0, 200).map(r => {
    const code = codeMap[r.student_id] || r.student_code || "";
    const displayName = formatStudentDisplay(r.student_name, code);
    return `<tr><td>${formatStudentCodeHtml(code)}${escapeHtml(displayName)}</td><td>${escapeHtml(r.group_name)}</td><td>${r.date}</td><td class="grade-cell-${r.grade.toLowerCase()}">${r.grade_label}</td></tr>`;
  }).join("");
...
function exportStudent() {
  ...
  window.open(`/api/export/student/${sid}?homework_type_id=${State.currentHomeworkTypeId}&start=${start}&end=${end}`, "_blank");
}

function exportClass() {
  const start = document.getElementById("exportStartDate").value;
  const end = document.getElementById("exportEndDate").value;
  window.open(`/api/export/class?homework_type_id=${State.currentHomeworkTypeId}&start=${start}&end=${end}&class_id=${State.currentClassId}`, "_blank");
}
...
async function loadAnalytics() {
  ...
  // 同步 analytics 页面的种类下拉框
  const analyticsSel = document.getElementById('analyticsTypeSelect');
  if (analyticsSel && parseInt(analyticsSel.value) !== tid) {
    analyticsSel.value = tid;
  }
  ...
  document.getElementById("scSubmitted").textContent = submitRate + "%";
  document.getElementById("scARate").textContent = aRate + "%";
  document.getElementById("scBRate").textContent = bRate + "%";
  document.getElementById("scCRate").textContent = cRate + "%";
  document.getElementById("scMissing").textContent = missing;
  document.getElementById("scAvgScore").textContent = avgScore;

  // 饼图 — v2: 数据哈希比较避免无意义重建
  const pieData = [d.grade_counts.A || 0, d.grade_counts.B || 0, d.grade_counts.C || 0, missing];
  ...
  chartPie = new Chart(pieCtx, {
    type: "doughnut",
    data: {
      labels: ["A (优秀)", "B (良好)", "C (待提升)", "未交"],
      datasets: [{
        data: pieData,
        backgroundColor: ["#A8D5BA", "#7EB5D6", "#F4C97E", "#E8A0BF"],
...
function animateValue(id, target) {
  ...
  if (isNaN(current) || isNaN(target)) { el.textContent = target; return; }  // v2: NaN 防护
  ...
  body.innerHTML = ranking.map((g, i) => `
    <div class="ranking-item">
      ...
      <div class="ranking-bar-wrap">
        <div class="ranking-bar-fill" style="width:0%;background:${g.color}" data-target-width="${g.a_rate}">
...
function renderStudentAlerts(data) {
  ...
  const codeMap = {};
  State.students.forEach(s => { codeMap[s.id] = s.student_code || ""; });
  const getDisplay = (s) => formatStudentDisplay(s.student_name, codeMap[s.student_id] || "");
  ...
  if (data.at_risk.length === 0) {
    ...
    html += data.at_risk.map(s => `
      <div class="alert-student-item risk" data-sid="${s.student_id}" data-sname="${escapeHtml(getDisplay(s))}">
        <span class="alert-student-name${privacyClass()}">${formatStudentCodeHtml(codeMap[s.student_id] || "")}${escapeHtml(getDisplay(s))}</span>
        <span class="alert-student-group">${escapeHtml(s.group_name)}</span>
        <span class="alert-student-tag risk">连续${s.consecutive_x}次未交</span>
        <span class="alert-student-grades">${s.last_grades.map(g => `<span class="g-${g}">${g==='X'?'未交':g}</span>`).join('')}</span>
      </div>
    `).join('');
  }
  html += '</div>';
  ...
  if (data.improving.length === 0) {
    ...
    html += data.improving.map(s => `
      <div class="alert-student-item improve" data-sid="${s.student_id}" data-sname="${escapeHtml(getDisplay(s))}">
        <span class="alert-student-name${privacyClass()}">${formatStudentCodeHtml(codeMap[s.student_id] || "")}${escapeHtml(getDisplay(s))}</span>
        <span class="alert-student-group">${escapeHtml(s.group_name)}</span>
        <span class="alert-student-tag improve">${s.from_grade}→${s.to_grade} 进步</span>
        <span class="alert-student-grades">${s.recent_grades.map(g => `<span class="g-${g}">${g==='X'?'未交':g}</span>`).join('')}</span>
      </div>
    `).join('');
  }
  html += '</div>';
...
document.body.addEventListener('click', function(e) {
  ...
  const date = document.getElementById('analyticsDate').value;
  API.get(`/api/analytics/trend-compare?period=${period}&class_id=${State.currentClassId}&homework_type_id=${State.currentHomeworkTypeId}`).then(res => {
...
let mobileLastScanTs = "1970-01-01 00:00:00";
...
async function onPcScanSuccess(decodedText) {
  if (!pcScanning) return;
  pcScanning = false;
  const code = decodedText.trim();

  try {
    const res = await API.get(`/api/student/by-code/${encodeURIComponent(code)}?class_id=${State.currentClassId}`);
    if (res.code === 0) {
      const s = res.data;
      const displayName = formatStudentDisplay(s.name, s.student_code || code);
      const grade = currentScanMode === "batch" ? currentScanGrade : null;

      if (currentScanMode === "single") {
        const chosen = prompt(`学生：${displayName} (${s.student_code || code})\n请输入等级 (A/B/C/X)：`, "A");
        if (chosen && ["A","B","C","X"].includes(chosen.toUpperCase())) {
          await API.post("/api/scan/single", { student_code: code, grade: chosen.toUpperCase(), date: document.getElementById("homeworkDate").value, class_id: State.currentClassId, homework_type_id: State.currentHomeworkTypeId });
          showToast(`${displayName} → ${chosen.toUpperCase()}`, "success");
          debouncedLoadStats();
        }
      } else {
        addPendingScan(code, displayName, grade, s.id, false);
        showToast(`${displayName} → ${grade === "X" ? "未交" : grade}`, "info");
      }
...
function renderPendingList() {
  ...
  list.innerHTML = pendingScans.map((p, i) => {
    ...
    <div class="pending-item ${p.external ? 'external' : ''}">
      <span class="pi-code">${escapeHtml(p.student_code)}</span>
      <span class="pi-name${privacyClass()}">${formatStudentCodeHtml(p.student_code)}${escapeHtml(displayName)}</span>
      ${p.external ? '<span class="pi-external">⚠ 非本班</span>' : ''}
      <span class="pi-grade grade-${(p.grade||'x').toLowerCase()}">${p.grade === "X" ? "未交" : (p.grade||"?")}</span>
...
document.getElementById("btnConfirmScans").addEventListener("click", async () => {
  ...
  const res = await API.post("/api/scan/batch", {
    ...
    records: valid.map(p => ({ student_code: p.student_code, grade: p.grade })), homework_type_id: State.currentHomeworkTypeId,
...
document.getElementById("btnGenPairQR").addEventListener("click", async () => {
  ...
  if (res.code === 0) {
    ...
    if (typeof QRCode !== "undefined") {
      new QRCode(box, { text: url, width: 150, height: 150, colorDark: "#5D5A5A", colorLight: "#ffffff" });
    } else {
      const script = document.createElement("script");
...
function startMobilePolling() {
  ...
  mobilePollTimer = setInterval(pollMobileScans, 1500);
}
...
async function pollMobileScans() {
  try {
    const res = await API.get(`/api/mobile/scans?since=${encodeURIComponent(mobileLastScanTs)}&class_id=${State.currentClassId}`);
...
document.getElementById("btnRefreshMobile").addEventListener("click", () => {
  mobileLastScanTs = "1970-01-01 00:00:00";
  pollMobileScans();
});
...
async function openDetailModal(type) {
  ...
  if (type === "all") {
    ...
    const res = await API.get(`/api/analytics/submitted?date=${date}&class_id=${State.currentClassId}&homework_type_id=${State.currentHomeworkTypeId}`);
    if (res.code === 0) {
      ...
      list.innerHTML = res.data.map(s => {
        ...
        const displayName = formatStudentDisplay(s.student_name, code);
        return `
        <div class="detail-item" data-sid="${s.student_id}" data-sname="${escapeHtml(displayName)}" style="cursor:pointer;">
          <span class="detail-name${privacyClass()}">${formatStudentCodeHtml(code)}${escapeHtml(displayName)}</span>
    ...
    const res = await API.get(`/api/analytics/missing?date=${date}&class_id=${State.currentClassId}&homework_type_id=${State.currentHomeworkTypeId}`);
    if (res.code === 0) {
    ...
    const res = await API.get(`/api/analytics/submitted?date=${date}&class_id=${State.currentClassId}&homework_type_id=${State.currentHomeworkTypeId}&grade=${grade}`);
    if (res.code === 0) {
      const gradeCount = res.total;
      const overview = await API.get(`/api/analytics/overview?date=${date}&class_id=${State.currentClassId}&homework_type_id=${State.currentHomeworkTypeId}`);
...
function exportStudentReport() {
  ...
  const start = document.getElementById("exportStartDate")?.value || "";
  const end = document.getElementById("exportEndDate")?.value || "";
  let url = `/api/export/student/${_currentReportSid}?homework_type_id=${State.currentHomeworkTypeId}`;
...
/** 隐私模式辅助函数：根据 State.privacyMode 返回合适的显示文本 */
function formatStudentDisplay(name, code) {
  if (State.privacyMode) return code || "???";
  return name;
}

/** 生成学号前缀HTML（正常模式显示，隐私模式不显示） */
function formatStudentCodeHtml(code) {
  if (State.privacyMode || !code) return "";
...
/** 生成隐私模式的CSS class */
function privacyClass() {
  return State.privacyMode ? " privacy-mode" : "";
}
...
document.addEventListener("keydown", e => {
  ...
  if (e.ctrlKey && e.key === "3") { e.preventDefault(); switchTab("export"); }
