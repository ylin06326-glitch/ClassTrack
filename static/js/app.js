...
/* ============================================================
   滑块拖动切换 Tab (Apple Design: Pointer Events + Momentum + Spring)
   ============================================================ */
(function initTabDrag() {
  ...
  // 速度历史：记录最后几个 pointermove 的位置和时间
  const velocityHistory = [];
  const VELOCITY_SAMPLES = 5;

  function getTabButtons() {
    return Array.from(nav.querySelectorAll(".tab-btn")).filter(b => b.dataset.tab);
  }

  function getTabCenter(btn) {
    const navRect = nav.getBoundingClientRect();
    const btnRect = btn.getBoundingClientRect();
    return btnRect.left - navRect.left + btnRect.width / 2;
  }

  function findNearestTab(x) {
    const buttons = getTabButtons();
    let nearest = buttons[0];
    let minDist = Infinity;
    buttons.forEach(btn => {
      const center = getTabCenter(btn);
  ...
  // Apple 动量预测公式：指数衰减
  // project(initialVelocity, decelerationRate) = (v/1000) * d / (1 - d)
  ...
  function recordVelocity(clientX, timestamp) {
    velocityHistory.push({ x: clientX, t: timestamp });
  ...
  function onPointerDown(e) {
    ...
    startX = e.clientX;
    const transformMatch = indicator.style.transform.match(/translateX\(([-\d.]+)px\)/);
    startTransform = transformMatch ? parseFloat(transformMatch[1]) : 0;
    currentTransform = startTransform;
    velocityHistory.length = 0;
    recordVelocity(e.clientX, performance.now());
    e.preventDefault();
  }
  ...
  function onPointerMove(e) {
    ...
    recordVelocity(clientX, performance.now());
    const deltaX = clientX - startX;
    currentTransform = startTransform + deltaX;
    // 边界橡皮筋效果（rubber-banding）
    const navWidth = nav.offsetWidth;
    const indicatorWidth = indicator.offsetWidth;
    const maxTransform = navWidth - indicatorWidth - 8;
    const minTransform = 4;
    if (currentTransform > maxTransform) {
      // 超出右边界，橡皮筋减速
      const overshoot = currentTransform - maxTransform;
      currentTransform = maxTransform + rubberband(overshoot, navWidth);
    } else if (currentTransform < minTransform) {
      // 超出左边界，橡皮筋减速
      const overshoot = minTransform - currentTransform;
      currentTransform = minTransform - rubberband(overshoot, navWidth);
    }
    indicator.style.transform = `translateX(${currentTransform}px)`;
  }
  ...
  function onPointerUp(e) {
    ...
    // 用动量预测停止位置
    const indicatorCenter = currentTransform + indicator.offsetWidth / 2;
    const projectedCenter = indicatorCenter + projectMomentum(velocity);
...
(function initGuide() {
  ...
  const dots = dotsContainer.querySelectorAll(".guide-dot");
  ...
  function goToStep(index) {
    ...
    nextBtn.textContent = currentStep === totalSteps - 1 ? "开始使用" : "下一步";
  }
  ...
  function hideGuide() {
    ...
    localStorage.setItem("classtrack_guide_shown", "true");
  }
  ...
  // 首次使用自动弹出
  if (!localStorage.getItem("classtrack_guide_shown")) {
    setTimeout(showGuide, 800);
  }

  // 暴露全局方法，方便设置页面添加"重新查看引导"按钮
